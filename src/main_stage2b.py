...
from src.config import MODELS, LABELS_S2B, ALPHA, RESULTS_DIR_S2B, CLASSICAL_MODELS
from src.features.extractors import CORE_GROUPS_S2B, CANDIDATE_GROUPS_S2B
from src.data.loader import build_dataset_stage2b
...
def main():
    print("\n"+"="*70); print(f"PART 1: MODEL SELECTION on core set {CORE_GROUPS_S2B}"); print("="*70)

    Xb_core, Xf_core, y_core, g_core = build_dataset_stage2b(CORE_GROUPS_S2B)
    print(f"Dataset: {Xb_core.shape}, {Xf_core.shape}, classes: {len(set(y_core))}")

    model_results_core = {}
    for model in MODELS:
        print(f"\n  {model}:")
        metrics = run_on_model(model, Xb_core, Xf_core, y_core, g_core, LABELS_S2B, epochs=150, hidden=16, stage='s2b')
        print_seeds(model, metrics)
        model_results_core[model] = metrics

    model_stage2b = pick_best_model(model_results_core)
    print(f"\n  >>> Model selected on core set: {model_stage2b}")

    print("\n"+"="*70); print(f"PART 2: GREEDY FORWARD FEATURE SELECTION on {model_stage2b}"); print("="*70)

    current_groups = list(CORE_GROUPS_S2B)
    remaining_candidates = list(CANDIDATE_GROUPS_S2B)

    Xb_cur, Xf_cur, y_cur, g_cur = build_dataset_stage2b(current_groups)
    current_metrics = run_on_model(model_stage2b, Xb_cur, Xf_cur, y_cur, g_cur, LABELS_S2B, epochs=150, hidden=16, stage='s2b')
    ...
    while remaining_candidates:
        ...
        for cand in remaining_candidates:
            ...
            Xb_t, Xf_t, y_t, g_t = build_dataset_stage2b(trial_groups)
            trial_metrics = run_on_model(model_stage2b, Xb_t, Xf_t, y_t, g_t, LABELS_S2B, epochs=150, hidden=16, stage='s2b')
    ...
    FINAL_GROUPS_S2B = current_groups
    print(f"\n  >>> FINAL FEATURE SET (Stage 2b, model={model_stage2b}): {FINAL_GROUPS_S2B}")

    Xb_final, Xf_final, y_final, g_final = build_dataset_stage2b(FINAL_GROUPS_S2B)
    FINAL_FEATURE_NAMES = feature_names_for(FINAL_GROUPS_S2B, stage='s2b')

    print("\n"+"="*70); print(f"PART 2b: RE-VERIFY MODEL on final feature set {FINAL_GROUPS_S2B}"); print("="*70)

    model_results_final = {model_stage2b: current_metrics} 
    for model in MODELS:
        if model == model_stage2b: continue
        print(f"\n  {model}:")
        metrics = run_on_model(model, Xb_final, Xf_final, y_final, g_final, LABELS_S2B, epochs=150, hidden=16, stage='s2b')
        print_seeds(model, metrics)
        model_results_final[model] = metrics

    best_model = pick_best_model(model_results_final)
    if best_model != model_stage2b:
        print(f"\n  >>> Model ranking CHANGED with final feature set: {model_stage2b} -> {best_model}")
    else:
        print(f"\n  >>> Model choice confirmed: {best_model}")

    FINAL_Xb, FINAL_Xf, FINAL_y, FINAL_g = Xb_final, Xf_final, y_final, g_final
    FINAL_FEATURE_SET = f"{FINAL_GROUPS_S2B} ({Xf_final.shape[1]}-dim)"

    print("\n"+"="*70); print("PART 3: PER-CLASS PERFORMANCE (final config)"); print("="*70)

    all_true, all_pred = get_loso_predictions(best_model, FINAL_Xb, FINAL_Xf, FINAL_y, FINAL_g, LABELS_S2B, epochs=150, hidden=16, stage='s2b')
    ...
    for i, cls in enumerate(LABELS_S2B):
        ...
        support = sum(1 for t in all_true if t == cls)
        per_class_metrics[cls] = {'precision': precision, 'recall': recall, 'f1': f1, 'support': support}
        print(f"  {cls:<10} {precision:.4f}      {recall:.4f}      {f1:.4f}")

    print("\n"+"="*70); print("PART 4: VISUALIZATION"); print("="*70)

    plot_confusion_matrix(cm, best_model, FINAL_FEATURE_SET, stage='s2b')
    plot_feature_distributions(best_model, FINAL_Xf, FINAL_y, FINAL_FEATURE_NAMES, stage='s2b')
    plot_energy_profiles(FINAL_Xb, FINAL_y, stage='s2b')
    ...
    if best_model in CLASSICAL_MODELS:
        ...
        clf = get_classical_models(0, stage='s2b')[best_model]
    ...
    results = {
        'model_selected_on_core': model_stage2b,
        'final_feature_groups': FINAL_GROUPS_S2B,
        'final_model': best_model,
        'model_changed_after_feature_selection': best_model != model_stage2b,
    ...
    print(f"  Model selected on core set: {model_stage2b}")
    print(f"  Final feature groups (greedy, significance-gated): {FINAL_GROUPS_S2B}")
