...
class StreamingFusionNet(nn.Module):
    def __init__(self, per_bin_dim, flat_dim, n_classes, inner_hidden=16, outer_hidden=16, mlp_hidden=32):
        ...
        self.mlp = nn.Sequential(nn.Linear(flat_dim, mlp_hidden), nn.ReLU(), nn.Dropout(0.3), nn.Linear(mlp_hidden, outer_out))
        self.q_proj_dl=nn.Linear(outer_out,outer_out); self.k_proj_ml=nn.Linear(outer_out,outer_out); self.v_proj_ml=nn.Linear(outer_out,outer_out)
        self.q_proj_ml=nn.Linear(outer_out,outer_out); self.k_proj_dl=nn.Linear(outer_out,outer_out); self.v_proj_dl=nn.Linear(outer_out,outer_out)
        self.dropout = nn.Dropout(0.4)
    ...
    def forward(self, x_bins, x_flat):
        ...
        dl_out, _ = self.outer_lstm(ws); dl_out = self.dropout(dl_out)
        ml_out = self.mlp(x_flat.view(b*nw, -1)).view(b, nw, -1); ml_out = self.dropout(ml_out)
        q_dl=self.q_proj_dl(dl_out); k_ml=self.k_proj_ml(ml_out); v_ml=self.v_proj_ml(ml_out)
        attn_dl = torch.softmax((q_dl*k_ml).sum(-1,keepdim=True)/(q_dl.size(-1)**0.5), dim=-1)
        dl_att = attn_dl*v_ml + dl_out
        q_ml=self.q_proj_ml(ml_out); k_dl=self.k_proj_dl(dl_out); v_dl=self.v_proj_dl(dl_out)
        attn_ml = torch.softmax((q_ml*k_dl).sum(-1,keepdim=True)/(q_ml.size(-1)**0.5), dim=-1)
        ml_att = attn_ml*v_dl + ml_out
        return self.classifier(torch.cat([dl_att, ml_att], dim=-1)).squeeze(0)

def train_streaming_fusion(train_data, l2i, n_classes, per_bin_dim, flat_dim, epochs, device):
    ...
    for epoch in range(epochs):
        for subj, (Xb, Xf, yw, p) in train_data.items():
            ...
            opt.zero_grad(); loss = F.cross_entropy(model(Xb_t, Xf_t), y_t); loss.backward(); opt.step()
    return model
