...
from src.config import MODELS, LABELS_S2A, ALPHA, RESULTS_DIR_S2A, CLASSICAL_MODELS
from src.features.extractors import CORE_GROUPS_S2A, CANDIDATE_GROUPS_S2A
from src.data.loader import build_dataset_stage2a
...
def main():
    print("\n"+"="*70); print(f"PART 1: MODEL SELECTION on core set {CORE_GROUPS_S2A}"); print("="*70)

    Xb_core, Xf_core, y_core, g_core = build_dataset_stage2a(CORE_GROUPS_S2A)
    print(f"Dataset: {Xb_core.shape}, {Xf_core.shape}, classes: {sorted(set(y_core))}")

    model_results_core = {}
    for model in MODELS:
        print(f"\n  {model}:")
        metrics = run_on_model(model, Xb_core, Xf_core, y_core, g_core, LABELS_S2A, epochs=300, stage='s2a')
        print_seeds(model, metrics)
        model_results_core[model] = metrics

    model_stage2a = pick_best_model(model_results_core)
    print(f"\n  >>> Model selected on core set: {model_stage2a}")

    print("\n"+"="*70); print(f"PART 2: GREEDY FORWARD FEATURE SELECTION on {model_stage2a}"); print("="*70)

    current_groups = list(CORE_GROUPS_S2A)
    remaining_candidates = list(CANDIDATE_GROUPS_S2A)

    Xb_cur, Xf_cur, y_cur, g_cur = build_dataset_stage2a(current_groups)
    current_metrics = run_on_model(model_stage2a, Xb_cur, Xf_cur, y_cur, g_cur, LABELS_S2A, epochs=300, stage='s2a')
    ...
    while remaining_candidates:
        ...
        for cand in remaining_candidates:
            ...
            Xb_t, Xf_t, y_t, g_t = build_dataset_stage2a(trial_groups)
            trial_metrics = run_on_model(model_stage2a, Xb_t, Xf_t, y_t, g_t, LABELS_S2A, epochs=300, stage='s2a')
    ...
    FINAL_GROUPS_S2A = current_groups
    print(f"\n  >>> FINAL FEATURE SET (Stage 2a, model={model_stage2a}): {FINAL_GROUPS_S2A}")

    Xb_final, Xf_final, y_final, g_final = build_dataset_stage2a(FINAL_GROUPS_S2A)
    FINAL_FEATURE_NAMES = feature_names_for(FINAL_GROUPS_S2A, stage='s2a')

    print("\n"+"="*70); print(f"PART 2b: RE-VERIFY MODEL on final feature set {FINAL_GROUPS_S2A}"); print("="*70)

    model_results_final = {model_stage2a: current_metrics} 
    for model in MODELS:
        if model == model_stage2a: continue
        print(f"\n  {model}:")
        metrics = run_on_model(model, Xb_final, Xf_final, y_final, g_final, LABELS_S2A, epochs=300, stage='s2a')
        print_seeds(model, metrics)
        model_results_final[model] = metrics

    best_model = pick_best_model(model_results_final)
    if best_model != model_stage2a:
        print(f"\n  >>> Model ranking CHANGED with final feature set: {model_stage2a} -> {best_model}")
    else:
        print(f"\n  >>> Model choice confirmed: {best_model}")

    FINAL_Xb, FINAL_Xf, FINAL_y, FINAL_g = Xb_final, Xf_final, y_final, g_final
    FINAL_FEATURE_SET = f"{FINAL_GROUPS_S2A} ({Xf_final.shape[1]}-dim)"

    print("\n"+"="*70); print("PART 3: PER-CLASS PERFORMANCE (final config)"); print("="*70)

    all_true, all_pred = get_loso_predictions(best_model, FINAL_Xb, FINAL_Xf, FINAL_y, FINAL_g, LABELS_S2A, epochs=300, stage='s2a')
    ...
    plot_confusion_matrix(cm, best_model, FINAL_FEATURE_SET, stage='s2a')
    plot_feature_distributions(best_model, FINAL_Xf, FINAL_y, FINAL_FEATURE_NAMES, stage='s2a')
    plot_energy_profiles(FINAL_Xb, FINAL_y, stage='s2a')
    ...
    if best_model in CLASSICAL_MODELS:
        ...
        clf = get_classical_models(0, stage='s2a')[best_model]
    ...
    results = {
        'model_selected_on_core': model_stage2a,
        'final_feature_groups': FINAL_GROUPS_S2A,
        'final_model': best_model,
        'model_changed_after_feature_selection': best_model != model_stage2a,
    ...
    print(f"  Model selected on core set: {model_stage2a}")
    print(f"  Final feature groups (greedy, significance-gated): {FINAL_GROUPS_S2A}")
